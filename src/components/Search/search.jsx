...
export default ({data}) => {
	...
	const search = (e) => {
		...
		let arr = data.filter(el => el.name.toLowerCase().includes(e.target.value.toLoverCase()));
		setSearchData(arr);
	}
	...
	{searchData.length > 0 ? `найдено ${searchData.length} товаров` : `товаров не найдено`}</div>}	
	</div>
}